...
using MyId = Id<int, 1, 2, 3, 17>;


// This struct allows us to use std::is_invocable_v to test if some calls to MyId::Make() do not
// compile without actually throwing compiler errors. See CallSpan in Span.test.cpp for an
// explanation.
struct CallMyIdMake
{
    template<int t>
...
TEST_CASE("Id")
{
    ...
    for(auto i : {-1, 0, 16, 123456})
    {
        ...
        CHECK(makeResult.error() == ErrorCode::invalidValue);
    }
}
