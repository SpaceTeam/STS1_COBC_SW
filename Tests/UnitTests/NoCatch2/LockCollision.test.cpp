...
private:
    ...
    void run() override
    {
        ...
        if(strcmp(this->name, "Thread 1") == 0)
        {
            ...
            PRINTF("%s: Try format\n", this->name);
            while((errorCode = lfs_format(&lfs, &sts1cobcsw::fs::lfsConfig))
                  == static_cast<int>(fs::ErrorCode::lfsLockBusy))
            {
            ...
            PRINTF("%s: Try mount\n", this->name);
            while((errorCode = lfs_mount(&lfs, &sts1cobcsw::fs::lfsConfig))
                  == static_cast<int>(fs::ErrorCode::lfsLockBusy))
            {
                RODOS::AT(RODOS::NOW() + retryDelay);
                // yield();
        ...
        while((errorCode = lfs_mkdir(&lfs, directoryPath))
              == static_cast<int>(fs::ErrorCode::lfsLockBusy))
        {
            RODOS::AT(RODOS::NOW() + retryDelay);
            // yield();
        ...
        while((errorCode = lfs_file_close(&lfs, &file))
              == static_cast<int>(fs::ErrorCode::lfsLockBusy))
        {
            RODOS::AT(RODOS::NOW() + retryDelay);
            // yield();
        ...
        // while((errorCode = lfs_unmount(&lfs)) == static_cast<int>(fs::ErrorCode::lfsLockBusy))
        // {
        //     RODOS::AT(RODOS::NOW() + retryDelay);
        //     // yield();
        // }
        ...
        while((errorCode = lfs_file_open(&lfs, &file, filePath, LFS_O_RDONLY))
              == static_cast<int>(fs::ErrorCode::lfsLockBusy))
        {
            RODOS::AT(RODOS::NOW() + retryDelay);
            // yield();
