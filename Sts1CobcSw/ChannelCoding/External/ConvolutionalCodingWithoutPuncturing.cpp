...
ViterbiCodecWithoutPuncturing::ViterbiCodecWithoutPuncturing()
{
...
auto ViterbiCodecWithoutPuncturing::NumParityBits() const -> std::size_t
{
    return polynomials.size();
}

auto ViterbiCodecWithoutPuncturing::NextState(unsigned int currentState, unsigned int input) const
    -> unsigned int
{
    return (currentState >> 1U) | (input << (constraint - 2U));
}

auto ViterbiCodecWithoutPuncturing::Output(unsigned int currentState, unsigned int input) const
    -> std::uint8_t
{
    return outputs[currentState | (input << (constraint - 1U))];
}

auto ViterbiCodecWithoutPuncturing::ProcessTwoBits(std::uint8_t bit1, std::uint8_t bit2)
    -> std::uint8_t
{
...
auto ViterbiCodecWithoutPuncturing::Encode(std::span<Byte const> data, [[maybe_unused]] bool flush)
    -> etl::vector<Byte, ViterbiCodecWithoutPuncturing::maxSize>
{
#ifdef DISABLE_CHANNEL_CODING
    auto dst = etl::vector<Byte, ViterbiCodecWithoutPuncturing::maxSize>();
...
auto ViterbiCodecWithoutPuncturing::InitializeOutputs() -> void
{
