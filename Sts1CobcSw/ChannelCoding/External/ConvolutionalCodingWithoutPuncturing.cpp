...
auto ViterbiCodec::NextState(unsigned int currentState, unsigned int input) const
    -> unsigned int
{
    return (currentState >> 1U) | (input << (constraint - 2U));
}

auto ViterbiCodec::Output(unsigned int currentState, unsigned int input) const
    -> std::uint8_t
{
    return outputs[currentState | (input << (constraint - 1U))];
}

auto ViterbiCodec::ProcessTwoBits(std::uint8_t bit1, std::uint8_t bit2)
    -> std::uint8_t
{
