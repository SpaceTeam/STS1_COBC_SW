    ...
    -> Result<DumpRawMemoryDataRequest>
{
    ...
    (void)DeserializeFrom<sts1cobcsw::ccsdsEndianness>(buffer.data(), &request.nDataAreas);
    ...
    if(not(buffer.size()
           == minApplicationDataLength
                  + totalSerialSize<DumpRawMemoryDataArea> * request.nDataAreas))
    {
        return ErrorCode::invalidDataLength;
    }
    (void)DeserializeFrom<sts1cobcsw::ccsdsEndianness>(
        buffer.subspan(minApplicationDataLength).data(), &request.dataAreas);
    ...
    -> Result<ReportParameterValuesRequest>
{
    ...
    (void)DeserializeFrom<sts1cobcsw::ccsdsEndianness>(buffer.data(), &request.nParameters);
    ...
    (void)DeserializeFrom<sts1cobcsw::ccsdsEndianness>(
        buffer.subspan(minApplicationDataLength).data(), &request.parameterIds);
    return request;
}
    ...
    -> Result<SetParameterValuesRequest>
{
    ...
    if(not(buffer.size()
           == minApplicationDataLength + totalSerialSize<Parameter> * request.nParameters))
    {
        return ErrorCode::invalidDataLength;
    }
    request.parameters.resize(request.nParameters);
    (void)DeserializeFrom<sts1cobcsw::ccsdsEndianness>(
        buffer.subspan(minApplicationDataLength).data(), &request.parameters);
    return request;
}
    ...
    -> Result<DeleteAFileRequest>
{
    ...
    // TODO: Add check for null terminator in etl::string DeserializeFrom
...
[[nodiscard]] auto ParseAsCopyAFileRequest(std::span<Byte const> buffer) -> Result<CopyAFileRequest>
{
    ...
    if(not(buffer.size() == applicationDataLength))
    {
        return ErrorCode::invalidDataLength;
    }
    (void)DeserializeFrom<sts1cobcsw::ccsdsEndianness>(buffer.data(), &request);
