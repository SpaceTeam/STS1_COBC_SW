...
// FIXME: Look up the correct hal::rfNirqPin!
auto nirqGpioPin = hal::GpioPin(hal::rfNirqPin);
// FIXME: Look up the correct hal::rfSdnPin!
auto sdnGpioPin = hal::GpioPin(hal::rfSdnPin);
// FIXME: Look up the correct hal::rfGpio0Pin!
auto gpio0GpioPin = hal::GpioPin(hal::rfGpio0Pin);
// FIXME: Look up the correct hal::rfGpio1Pin!
...
{
    // TODO: Don't forget that WDT_Clear has to be triggered regularely for the TX to work! (even without the watchdog timer on the PCB it needs to be triggered at least once after boot to enable the TX)
    ...
    // TODO: Check if correct SPI Pins are configured
    ...
    sdnGpioPin.Reset();
    RODOS::AT(RODOS::NOW() + 20 * RODOS::MILLISECONDS);
    ...
    sendCommand(sendBuffer, 7, nullptr, 0);
    ...
    sendBuffer[6] = 0x00;
    sendBuffer[7] = 0x00;
    sendCommand(sendBuffer, 8, nullptr, 0);
    ...
    sendBuffer[7] = 0x0F; // No extended RX preamble timeout, 0x0f nibbles timeout until detected preamble is discarded as invalid
    sendBuffer[8] = 0x31; // First transmitted preamble bit is 1, unit of preampreamble TX length is in bytes
    ...
    sendCommand(sendBuffer, 13, nullptr, 0);
    ...
    sendBuffer[5] = 0b01011000; // Valid CCSDS TM sync word for Reed-Solomon or convolutional coding
    sendBuffer[6] = 0b11110011; // Be careful: Send order is MSB-first but Little endian so the lowest bit of the
    sendBuffer[7] = 0b00111111; // highest byte is transmitted first, which is different to how the CCSDS spec
    sendBuffer[8] = 0b10111000; // annotates those bit patterns!
    sendCommand(sendBuffer, 9, nullptr, 0);
    ...
    sendBuffer[7] = 0x01; // Don't split RX and TX field information (length, ...), enable RX packet handler, use normal (2)FSK, no Manchester coding, no CRC, data transmission with MSB first.
    sendCommand(sendBuffer, 8, nullptr, 0);
    ...
    sendBuffer[5] = 0x00;
    sendBuffer[6] = 0x00;
    sendBuffer[7] = 0x30; // Trigger TX FiFo almost empty interrupt when 0x30 bytes in FiFo (size 0x40) are empty
    sendBuffer[8] = 0x30; // Trigger RX FiFo almost full interrupt when 0x30 bytes in FiFo (size 0x40) are full
    ...
    sendCommand(sendBuffer, 16, nullptr, 0);
    ...
    // register value is calculated with (2^19 * outdiv * deviation_Hz)/(N_presc * F_xo) = 167.772 = 168 = 0x0000A8
    // 9k6 deviation = 671 = 0x29F
    ...
    sendBuffer[8] = 0x80;
    sendBuffer[9] = 0x00;
    sendBuffer[10] = 0x70; // Decimation NDEC0 = 0, NDEC1 = decimation by 8, NDEC2 = decimation by 2
    sendBuffer[11] = 0x20; // Normal decimate-by-8 filter gain, don't bypass the decimate-by-2 polyphase filter, bypass the decimate-by-3 polyphase filter, enable droop compensation, channel selection filter in normal mode (27 tap filter)
    sendCommand(sendBuffer, 12, nullptr, 0);

    // RF Modem BCR Oversampling Rate, Modem BCR NCO Offset, Modem BCR Gain, Modem BCR Gear & Modem BCR Misc
    sendBuffer[0] = 0x11;
    sendBuffer[1] = 0x20;
    sendBuffer[2] = 0x09;
    sendBuffer[3] = 0x22;
    sendBuffer[4] = 0x03; // RX symbol oversampling rate of 0x30D/8 = 781/8 = 97.625
    sendBuffer[5] = 0x0D; // (According to the datasheet usual values are in the range of 8 to 12 where this value seems to be odd?)
    ...
    sendBuffer[12] = 0xC2; // Stop NCO for one sample clock in BCR mid-point phase sampling condition to escape, disable NCO resetting in case of mid-point phase sampling condition, don't double BCR loop gain, BCR NCO compensation is sampled upon detection of the preamble end, disable NCO frequency compensation, bypass compensation term feedback to slicer, bypass compensation term feedback to BCR tracking loop
    sendCommand(sendBuffer, 13, nullptr, 0);
    ...
    sendBuffer[2] = 0x07;
    sendBuffer[3] = 0x2C;
    sendBuffer[4] = 0x04; // AFC_SLOW gain 4, AFC_FAST gain 0, Switch gear after detection of preamble
    sendBuffer[5] = 0x36; // LGWAIT = 6, SHWAIT = 3
    sendBuffer[6] = 0x80; // AFC loop gain = 0x003, don't half the loop gain, disable adaptive RX bandwidth, enable frequency error estimation
    sendBuffer[7] = 0x03;
    sendBuffer[8] = 0x30; // AFC limiter = 0x30AF
    sendBuffer[9] = 0xAF;
    sendBuffer[10] = 0x80; // Expected freq error is less then 12*symbol rate, AFC correction of PLL will be frozen if a consecutive string of 1s or 0s that exceed the serach period is encountered, don't switch clock source for frequency estimator, don't freeze AFC at preamble end, AFC correction uses freq estimation by moving average or minmax detector in async demod,disable AFC value feedback to PLL, freeze AFC after gear switching
    sendCommand(sendBuffer, 11, nullptr, 0);
    ...
    sendBuffer[2] = 0x01;
    sendBuffer[3] = 0x35;
    sendBuffer[4] = 0xE2; // reset peak detectors only on change of gain indicated by peak detector output, reduce ADC gain when AGC gain is at minimum, normal AGC speed, don't increase AGC gain during signal reductions in ant diversity mode, always perform gain decreases in 3dB steps instead of 6dB steps, AGC is enabled over whole packet length
    sendCommand(sendBuffer, 5, nullptr, 0);

    // RF Modem AGC Window Size, AGC RF Peak Detector Decay, AGC IF Peak Detector Decay, 4FSK Gain, 4FSK Slicer Threshold, 4FSK SYmbol Mapping Code, OOK Attack/Decay Times
    sendBuffer[0] = 0x11;
    sendBuffer[1] = 0x20;
    sendBuffer[2] = 0x09;
    sendBuffer[3] = 0x38;
    sendBuffer[4] = 0x11; // AGC gain settling window size = 1, AGC signal level measurement window = 1
    ...
    // RF Modem OOK Control, OOK Misc, RAW Search, RAW Control, RAW Eye, Antenna Diversity Mode, Antenna Diversity Control, RSSI Threshold
    sendBuffer[0] = 0x11;
    sendBuffer[1] = 0x20;
    sendBuffer[2] = 0x09;
    sendBuffer[3] = 0x42;
    sendBuffer[4] = 0xA4; // OOK Squelch off, OOK slicer output de-glitching by bit clock, raw output is synced to clock, MA_FREQUDOWN=0, AGC and OOK movign average detector threshold will be frozen after preamble detection, S2P_MAP=2
    sendBuffer[5] = 0x02; // OOK uses moving average detector, OOK peak detector discharge does not affect decay rate, disable OOK squelch, always discharge peak detector, normal moving average window
    sendBuffer[6] = 0xD6;
    ...
    sendBuffer[2] = 0x01;
    sendBuffer[3] = 0x4C;
    sendBuffer[4] = 0x00; // Disable RSSI latch, RSSI value is avg over last 4*Tb bit periods, disable RSSI threshold check after latch
    sendCommand(sendBuffer, 5, nullptr, 0);
    ...
    sendBuffer[2] = 0x01;
    sendBuffer[3] = 0x51;
    sendBuffer[4] = 0x0A; // Band = FVCO_DIV_8, high performance mode fixed prescaler div2, force recalibration
    sendCommand(sendBuffer, 5, nullptr, 0);
    ...
    sendBuffer[2] = 0x04;
    sendBuffer[3] = 0x00;
    sendBuffer[4] = 0x08; // PA switching amp mode, PA_SEL = HP_COARSE, disable power sequencing, disable external TX ramp signal
    // TODO: Tune output power to fully utilize the amplifier without clipping
    sendBuffer[5] = 0x0C; // Enabled PA fingers (sets output power but not linearly). See page 35 of the Si446x datasheet. We need around 6dBm to go close to the maximum of our amplifier (33dBm max output, ~20-27dBm gain on 433MHz).
    sendBuffer[6] = 0x00; // 10µA bias current per enabled finger, complementary drive signal with 50% duty cycle
    sendBuffer[7] = 0x3D; // Ramping time constant = 0x1d, FSK modulation delay 6µs
    sendCommand(sendBuffer, 8, nullptr, 0);

    // RF Synth Feed Forward Charge Pump Current, Integrated Charge Pump Current, VCO Gain Scaling Factor, FF Loop Filter Values
    sendBuffer[0] = 0x11;
    sendBuffer[1] = 0x23;
    sendBuffer[2] = 0x07;
    sendBuffer[3] = 0x00;
    ...
    sendBuffer[6] = 0x0B; // Set VCO scaling factor to maximum value, set tuning varactor gain to maximum value
    ...
    // Check Device
    sendBuffer[0] = 0x01;
    sendCommand(sendBuffer, 1, receiveBuffer, 9);
    uint16_t partInfo = receiveBuffer[1]<<8 | receiveBuffer[2];
    if (partInfo != 0x4463) {
        // There's something wrong!
	// TODO: What to do here?
        while(true){
            RODOS::AT(RODOS::NOW() + 500 * RODOS::MILLISECONDS);
        }
    }
}
...
auto Morse() -> void
{
    ...
    RODOS::AT(RODOS::NOW() + 1000 * RODOS::MILLISECONDS);
    while (true) {
        gpio0GpioPin.Reset();
        ...
        RODOS::AT(RODOS::NOW() + 10 * RODOS::MILLISECONDS);
        ...
        RODOS::AT(RODOS::NOW() + 10000 * RODOS::MILLISECONDS);
    }
}
