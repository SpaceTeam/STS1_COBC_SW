...
auto DeserializeFrom(void const * source, TelemetryRecord * data) -> void const *
{
    ...
    source = DeserializeFrom<endianness>(source, &(data->lastRequestId));
    return source;
}
...
auto SerializeTo(void * destination, TelemetryRecord const & data) -> void *
{
    ...
    destination = SerializeTo<endianness>(destination, data.lastRequestId);
    return destination;
}
