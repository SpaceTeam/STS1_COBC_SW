...
{
    lfs_info infoStruct{};
    auto statError = lfs_stat(&lfs, Path(path).append(".lock").c_str(), &infoStruct);
    auto isOpen = statError == 0;  // TODO: check if file is open
    if(isOpen)
    {
        return ErrorCode::fileLocked;
    }
    auto error = lfs_remove(&lfs, path.c_str());
    if(error == 0)
    {
        return outcome_v2::success();
    }
