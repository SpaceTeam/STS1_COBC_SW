...
auto Handle(LoadRawMemoryDataAreasRequest const & request) -> void;
auto Handle(DumpRawMemoryDataRequest const & request) -> void;
auto Handle(PerformAFunctionRequest const & request, RequestId const & requestId) -> void;
auto Handle(ReportParameterValuesRequest const & request) -> void;
auto Handle(SetParameterValuesRequest const & request) -> void;
auto Handle(DeleteAFileRequest const & request) -> void;
auto Handle(ReportTheAttributesOfAFileRequest const & request) -> void;
auto Handle(SummaryReportTheContentOfARepositoryRequest const & request) -> void;
auto Handle(CopyAFileRequest const & request) -> void;
...
auto HandleReceivedData() -> void
{
    ...
    auto result = [&]() -> Result<void>
    {
        ...
        // persistentVariables.Add<"nCorrectableUplinkErrors">(decodeResult.value());
...
auto HandleRequestFrame(tc::TransferFrame const & frame) -> void
{
    ...
    {
        persistentVariables.Store<"lastRequestIdWasInvalid">(parseAsRequestResult.error()
                                                             == ErrorCode::invalidMessageTypeId);
    ...
    persistentVariables.Store<"lastRequestIdWasInvalid">(false);
    persistentVariables.Store<"lastRequestId">(request.packetSecondaryHeader.messageTypeId.Value());
...
auto VerifyAndHandle(Request const & request, RequestId const & requestId) -> void
{
    ...
    if constexpr(std::is_same_v<decltype(parseFunction), decltype(&ParseAsPerformAFunctionRequest)>)
    {
